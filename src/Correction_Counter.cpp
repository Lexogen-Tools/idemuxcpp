...
void Correction_Counter::count_correction(string i7, string i5, string i1, bool corrected_i7, bool corrected_i5, bool corrected_i1){
...
void Correction_Counter::write_correction(string filename, unordered_map<string, string> &barcode_sample_map){
	...
	for(auto it = Counts_combination.begin(); it != Counts_combination.end(); it++){
		...
		if (it_combi != barcode_sample_map.end()){
			...
			csvfile << sample_name << "," << i7 << "," << i5 << "," << i1 << "," << it->second.all << "," << it->second.i7 << "," << it->second.i5 << "," << it->second.i1 << std::endl;
		}
		else{
