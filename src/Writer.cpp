 ...
 */
void
Writer::write_summary(std::unordered_map<string, size_t> &counter,
                      string                              output_dir)
{
...
struct {
  bool
  operator()(const pair<string, size_t> & a,
             const pair<string, size_t> & b) const
  {
...
void
Writer::write_barcode_summary(correction_counter  *counted_corrections_per_index,
                              std::string         barcode_corrections_file)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (unordered_map<string, size_t>::iterator it = count_map->begin(); it != count_map->end();
         it++)
