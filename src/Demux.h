...
int demux_list_reads(demux_thread_parameter *tp){
	...
	for (size_t i = 0; i < max_reads; i++){
		...
		s_barcodes = process_mate_pair(mate_pair, tp->i7_wanted, tp->i5_wanted,
				tp->i1_wanted, tp->map_i7, tp->map_i5, tp->map_i1, *tp->i7_i5_i1_info_map,
				processed_mates, tp->counted_corrections_per_index, tp->max_length_i7, tp->max_length_i5);
...
int compress_and_delete(string *string_to_write, vector<uint8_t> *compressed_memory){
	ZipFastqWriter::compress_memory((uint8_t*)string_to_write->c_str(), string_to_write->length(), compressed_memory, 4);
...
void demux(unordered_map<string, string> *barcode_sample_map,
		vector<Barcode*> &barcodes, string read1_file, string read2_file, unordered_map<string, i1_info> &i7_i5_i1_info_map,
		string output_dir, Parser &parser, size_t queue_size, size_t reading_threads, size_t compressing_threads, size_t processing_threads, string barcode_corrections_file,
                bool skip_check, bool restrict_barcode_length, double queue_buffer_gb, size_t gzip_block_size_bytes, bool single_end_mode, bool verbose) {
	...
	FileHandler *file_handler = new FileHandler(*barcode_sample_map, output_dir, paired_output);

	// then we iterate over all the paired end reads
	std::vector<std::pair<fq_read*, fq_read*>> *pe_reads = NULL;
	std::unordered_map<std::string, size_t> read_counter;

 	unordered_map<demux_thread_parameter*, std::future<int>>                        demux_threads;

	unordered_map<std::pair<compression_input,vector<uint8_t>*>*, std::future<int>> compress_threads;

	unordered_map<ZipFastqWriter*, string*> uncompressed_read_string_buffer;
	unordered_map<ZipFastqWriter*,vector<string*>> uncompressed_read_string_chunks;

	size_t compressed_block_id = 1;
	size_t last_compressed_block_id = compressed_block_id -1;
	...
	while (!finish_while) {
	    ...
	    for(auto it = uncompressed_read_string_chunks.begin(); it != uncompressed_read_string_chunks.end(); it++)
		    uncompressed_chunks += it->second.size();

		if(pe_reads == NULL && compress_threads.size() < compressing_threads && uncompressed_chunks < compressing_threads){
		...
		for(auto it_dt = demux_threads.begin(); it_dt != demux_threads.end(); it_dt++){
			...
			try {
				if(it_dt->second.valid()){
					is_done = it_dt->second.wait_for(std::chrono::nanoseconds(0)) == std::future_status::ready;
				}
			...
			if (is_done) {
        	  ...
        	  if(tp != NULL){
        		  if(tp->map_pairs != NULL){
					  for(auto it_mapped = tp->map_pairs->begin(); it_mapped != tp->map_pairs->end(); it_mapped++){
						  //results.push_back(std::pair(it_mapped->first, it_mapped->second));
						  vector<std::pair<fq_read*, fq_read*>>* reads = it_mapped->second;
						  std::pair<ZipFastqWriter*, ZipFastqWriter*>* writer_pair = it_mapped->first;
						  string* uncompressed_reads1 = NULL;
						  string* uncompressed_reads2 = NULL;
						  if(uncompressed_read_string_buffer.find(writer_pair->first) == uncompressed_read_string_buffer.end()){
						  ...
						  if(writer_pair->second != NULL){
							  if(uncompressed_read_string_buffer.find(writer_pair->second) == uncompressed_read_string_buffer.end()){
						  ...
						  for(auto it = reads->begin(); it != reads->end(); it++){
							 if(uncompressed_reads1->size() < gzip_block_size_bytes){
								 ...
								 uncompressed_reads1 = new string(it->first->to_string());
								 uncompressed_read_string_buffer[writer_pair->first] = uncompressed_reads1;
							 }
							 if(it->second != NULL)
							 {
								 if(uncompressed_reads2->size() < gzip_block_size_bytes){
									 ...
									 uncompressed_read_string_chunks[writer_pair->second].push_back(uncompressed_reads2);
									 uncompressed_reads2 = new string(it->second->to_string());
									 uncompressed_read_string_buffer[writer_pair->second] = uncompressed_reads2;
								 }
							 }
						  }
		...
		if(pe_reads != NULL && demux_threads.size() < processing_threads){
		 ...
		 for(auto it_ct = compress_threads.begin(); it_ct != compress_threads.end(); it_ct++){
		   ...
		   if (is_done){
			   ...
			   if(is_next_block){
				   ...
				   double seconds = std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count();
				   if (verbose){
					   string output_pattern = string("\rreads: %10.4g, reads demuxed: %10.4g, seconds: %10.4g, reads per second: %10.4g, ") +
							   string("uncompressed chunks: %lu, demux threads: %lu, compress threads: %lu") +
							   std::string(to_string(UINT64_MAX).length()*2 - to_string(uncompressed_chunks).length() - to_string(seconds).length(), ' ').c_str();

					   printf(output_pattern.c_str(),
							   reads_read, reads_demuxed, seconds, reads_demuxed/seconds, uncompressed_chunks, demux_threads.size(), compress_threads.size());
				   }
				   else{
					   printf("\rreads: %10.4g, reads demuxed: %10.4g, seconds: %10.4g, reads per second: %10.4g", reads_read, reads_demuxed, seconds, reads_demuxed/seconds);
				   }
				   fflush(stdout);
			   }
		   }
		 ...
		 if(finished_all_reading && demux_threads.size() == 0 && uncompressed_read_string_chunks.size() == 0 && uncompressed_read_string_buffer.size() > 0){
			 for(auto it_uc = uncompressed_read_string_buffer.begin(); it_uc != uncompressed_read_string_buffer.end(); it_uc++){
		 ...
		 while(uncompressed_read_string_chunks.size() > 0 && compress_threads.size() < compressing_threads){
			 ...
			 if(it_first_res != uncompressed_read_string_chunks.end()){
				 ...
				 if(reads->size() > 0)
				 {
					 vector<uint8_t> *compressed_memory = new vector<uint8_t>();
					 compression_input compression_in = {writer, compressed_block_id};
					 std::pair<compression_input,vector<uint8_t>*>* compressed_result = new std::pair<compression_input,vector<uint8_t>*>({compression_in, compressed_memory});
					 string *uc_data = reads->at(0);
					 compress_threads[compressed_result] = std::async(std::launch::async, compress_and_delete, uc_data, compressed_memory);
					 compressed_block_id++;
		 ...
		 finish_while = demux_threads.size() == 0 && compress_threads.size() == 0 && finished_all_reading && uncompressed_read_string_buffer.size() == 0 && uncompressed_read_string_chunks.size() == 0;
	}
	std::cout << std::endl;
	if(barcode_corrections_file.compare("") != 0){
