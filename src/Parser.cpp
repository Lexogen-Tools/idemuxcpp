...
Parser::parse_sample_sheet(string                           sample_sheet,
                           bool                             i5_rc,
                           bool                             i7_rc,
                           bool                             auto_detect,
                           ...
                           bool                             single_end_mode)
{
  ...
  for (size_t i = 1; i < csv_lines.size(); i++) {
    ...
    {
      i1_info i1_read_info =
      { default_i1_read, default_i1_start, int(default_i1_start + i1_bc.length()) };
      if (idx_i1_read >= 0) {
        i1_read_info.read_index = strtol(row[idx_i1_read].c_str(), NULL, 10);
        if (i1_read_info.read_index != 1 && i1_read_info.read_index != 2)
          throw(runtime_error(
                  "Error: the sample sheet contains an i1 read index which is neither 1 nor 2!"));

        if (i1_read_info.read_index != 1 && single_end_mode)
          throw(runtime_error(
                  "Error: no read 2 file given, but sample sheet contains an i1 read index in a read index other than 1!"));
      }
      ...
      if (idx_i1_start >= 0) {
        ...
        if (i1_read_info.start_index < 1)
          throw(runtime_error(
                  "Error: the sample sheet contains an i1 read start which is smaller than 1!"));
      ...
      if (it_i1_options != i7_i5_i1_info_map.end()) {
        ...
        string  message         =
          "Error: the i7,i5 combination occurs twice with different i1 read/start informations!";
        if (it_i1_options->second.read_index != i1_read_info.read_index) {
          ...
          message         += string_format("\nWrong i1 read indices %ld and %ld for barcodes %s!",
                                           it_i1_options->second.read_index,
                                           i1_read_info.read_index,
                                           barcodes_i7_i5.c_str());
        }

        if (it_i1_options->second.start_index != i1_read_info.start_index) {
          valid_duplicate = false;
          message         += string_format("\nWrong i1 read starts %ld and %ld for barcodes %s!",
                                           it_i1_options->second.start_index,
                                           i1_read_info.start_index,
                                           barcodes_i7_i5.c_str());
        }
 ...
 */
string
Parser::reverse_complement(string sequence)
{
 ...
 */
bool
Parser::has_valid_barcode_combinations(Barcode &i7,
                                       Barcode &i5,
                                       Barcode &i1)
{
  /*
 ...
 */
void
Parser::peek_into_fastq_files(PairedReader &                  get_pe_fastq,
                              bool                            has_i7,
                              bool                            has_i5,
                              bool                            has_i1,
...
void
Parser::check_mate_pair(std::pair<fq_read *, fq_read *> mate_pair,
                        bool                            has_i7,
                        bool                            has_i5,
                        bool                            has_i1,
                        vector<int> &                   i7_length,
...
void
Parser::check_mate2_length(fq_read  *mate2,
                           int      i1_start,
                           int      i1_end)
{
 ...
 */
void
Parser::check_fastq_headers(std::pair<fq_read *, fq_read *> mate_pair,
                            bool                            has_i7,
                            bool                            has_i5,
                            vector<int> &                   i7_length,
                            vector<int> &                   i5_length,
                            ...
                            size_t                          max_length_i5)
{
  ...
  if (!right_number_of_barcodes && expected_number > 0) {
    //not all(right_number_of_barcodes):
  ...
  if (has_i7 && has_i5) {
    if (std::find(i7_length.begin(), i7_length.end(),
                  (int)bcs_mate1.first.length()) == i7_length.end()
        || std::find(i5_length.begin(), i5_length.end(),
                     (int)bcs_mate1.second.length()) == i5_length.end()) {
      string message = string_format(
  ...
  if (!has_i7 && has_i5) {
    if (std::find(i5_length.begin(), i5_length.end(),
                  (int)bcs_mate1.first.length()) == i5_length.end()) {
      string message = string_format(
...
void
Parser::check_fastq_header(fq_read      *mate,
                           bool         has_i7,
                           bool         has_i5,
                           vector<int> &i7_length,
                           vector<int> &i5_length,
                           ...
                           size_t       max_length_i5)
{
  ...
  int                       number_bc_m1 =
    (bcs_mate1.first == "" || bcs_mate1.second == "") ? 1 : 2;
  ...
  // check if the header conforms to what was specified in the sample sheet
  if (number_bc_m1 != expected_number && expected_number > 0) {
    //not all(right_number_of_barcodes):
    string  example_header =
      expected_number == 2 ? example_header_2 : example_header_1;
